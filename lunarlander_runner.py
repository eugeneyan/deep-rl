...
def train_main(args):
    ...
    train_df.to_csv('./{}_{}_log.csv'.format(env_name, args.model), index=False)

    # Save memory bank and weights
    memory_bank.save_memory('{}_memory_bank'.format(env_name))
    model.save_model('{}_{}_weights'.format(env_name, args.model))
